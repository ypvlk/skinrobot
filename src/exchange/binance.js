...
module.exports = class Binance {
    ...
    backfillCandles(symbol, period, startTime) {
        return new Promise((resolve, reject) => {
            ...
            f(`${this.getBaseUrl()}/api/v1/klines?${query}`)
                .then(res => {
                ...
                .then(body => {
                    if (!Array.isArray(body)) {
                        throw `Binance Spot: Candle backfill error: ${JSON.stringify(body)}`;
                    }
                    ...
                    resolve(body.map(candle => {
                        return new Candlestick(
                            ...
                            candle[3],
                            candle[4],
                            Binance.formatVolume(candle[5]),
                            candle[6]
                        );
                    }))
                })
                .catch(err => {throw new Error(`Binance Spot: Candle backfill error: ${String(err)}`)})
        })        
    }
